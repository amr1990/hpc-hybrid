               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
 ...
 int rank, size;
    ...
    MPI_Comm_rank (MPI_COMM_WORLD, &rank);        // get current process id
    MPI_Comm_size (MPI_COMM_WORLD, &size);        // get number of processes
    MPI_Get_processor_name(hostname, &namelen);   // get CPU name                                     // temp accumulation buffer
    ...
    #pragma omp for schedule(guided, dataSizeY) reduction(+:sum) nowait
...
//        printf("\nRound = %d, position = %ld, partsize= %d, chunksize=%d pixels\n", c, position, partsize, chunksize);
	...
	int imageChunkSize = chunk * dataSizeX; 
        ...
        int *Re = malloc(sizeof(int) * imageChunkSize);
	int *Ge = malloc(sizeof(int) * imageChunkSize);
	int *Be = malloc(sizeof(int) * imageChunkSize);

        int *oR = malloc(sizeof(int) * imageChunkSize);
	int *oG = malloc(sizeof(int) * imageChunkSize);
	int *oB = malloc(sizeof(int) * imageChunkSize);

	MPI_Scatter(redIn, imageChunkSize, MPI_INT, Re, imageChunkSize, MPI_INT, 0, MPI_COMM_WORLD);
        MPI_Scatter(greenIn, imageChunkSize, MPI_INT, Ge, imageChunkSize, MPI_INT, 0, MPI_COMM_WORLD);
        MPI_Scatter(blueIn, imageChunkSize, MPI_INT, Be, imageChunkSize, MPI_INT, 0, MPI_COMM_WORLD);
        
	MPI_Scatter(redOut, imageChunkSize, MPI_INT, oR, imageChunkSize, MPI_INT, 0, MPI_COMM_WORLD);  
        MPI_Scatter(greenOut, imageChunkSize, MPI_INT, oG, imageChunkSize, MPI_INT, 0, MPI_COMM_WORLD);
        MPI_Scatter(blueOut, imageChunkSize, MPI_INT, oB, imageChunkSize, MPI_INT, 0, MPI_COMM_WORLD);	
        
        convolve2D(Re, oR, dataSizeX, chunk, kern->vkern, kern->kernelX, kern->kernelY);
        convolve2D(Ge, oG, dataSizeX,chunk, kern->vkern, kern->kernelX, kern->kernelY);
        convolve2D(Be, oB, dataSizeX,chunk, kern->vkern, kern->kernelX, kern->kernelY);
        
	MPI_Gather(oR, imageChunkSize, MPI_INT, fOutRed, imageChunkSize, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Gather(oG, imageChunkSize, MPI_INT,  fOutGreen, imageChunkSize, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Gather(oB, imageChunkSize, MPI_INT, fOutBlue, imageChunkSize, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Barrier(MPI_COMM_WORLD);
